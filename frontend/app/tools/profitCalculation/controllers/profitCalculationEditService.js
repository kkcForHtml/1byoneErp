define(
    ...
    function (angularAMD) {
        "use strict";
        angularAMD.service(
            'profitCalculationEditService',
            function ($q, $modal) {
                this.showDialog = function (model) {
                    return $modal
                        .open({
                            animation: true,
                            controller: "profitCalculationEditService",
                            backdrop: "static",
                            size: "84%", //lg,sm,md,llg,ssm
                            templateUrl: 'app/tools/profitCalculation/views/profitCalculationEdit.html?ver=' + _version_,
                            resolve: {
                                model: function () {
                                    return model;
                                }
        ...
        angularAMD.controller("profitCalculationEditService", function ($scope, amHttp, $confirm, model, $modalInstance, configService, httpService, Notification, transervice, $http, $q, $interval, commonService, $filter, gridDefaultOptionsService, messageService, $timeout) {
            //初始化默认数据
            $scope.model = {
                channelTotalList: {
                    data: []
            ...
            function init () {
            	$p.promise.then(function () {
            		console.log(model);
            		$scope.model = $.extend(true, {}, $scope.model,model);
            		$scope.cv.changeItemVolume();
            		$scope.cv.changePackageVolume();
            		$scope.cv.changeOutPackageVolume();
            		$scope.imgSrc = $scope.model.MAIN_IMAGE_URL;
            ...
            $scope.save = function () {
                ...
                delete s_data.channelTotalList; 
                delete s_data.b_channel;                
                delete s_data.g_product_sku;                
                delete s_data.g_product_sku_purchasing_price;                
                
                httpService.httpHelper(httpService.webApi.api, "tools/platformdata", "update", "POST", s_data).then(function (result) {
