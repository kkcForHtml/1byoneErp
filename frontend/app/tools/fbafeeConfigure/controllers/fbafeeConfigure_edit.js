...
define(
    ...
    function (angularAMD) {
        angularAMD.service(
            'fbafeeConfigure_edit_service',
            function ($q, $modal) {
                this.showDialog = function (model) {
                    return $modal
                        .open({
                            animation: true,
                            controller: "fbafeeConfigure_edit_Ctrl",
                            backdrop: "static",
                            size: "1100px",//lg,sm,md,llg,ssm
                            templateUrl: 'app/tools/fbafeeConfigure/views/fbafeeConfigure_edit.html?ver=' + _version_,
                            resolve: {
                                model: function () {
                                    return model;
                                }
        ...
        angularAMD.controller("fbafeeConfigure_edit_Ctrl", function ($scope, $confirm, $filter, $timeout, uiGridConstants, amHttp, httpService, model, $modalInstance, Notification, transervice, $http, $q, $interval, commonService, gridDefaultOptionsService, messageService) {
            if (model) {
                ...
                //$scope.stateList = $scope.model.rowEntity.fieldDataObjectMap['ISACTIVE'].list;
            }
            ...
            $scope.save = function () {
                ...
                return httpService.httpHelper(httpService.webApi.api, "tools/fbafee", "update?id=" + $scope.model.FBAFEERULE_ID, "POST", dataUpdate).then(function (datas) {
            ...
            //FBA费表
            $scope.fbaFeeDetailInit = function () {
                var pageSize = $scope.fbaFeeDetailOptions.paginationPageSize ? $scope.fbaFeeDetailOptions.paginationPageSize : 5;
                var currentPage = $scope.fbaFeeDetailOptions.paginationCurrentPage ? $scope.fbaFeeDetailOptions.paginationCurrentPage : 1;
                var datas = {
                    "where": ['and', ['=', 'FBAFEERULE_ID', $scope.model.FBAFEERULE_ID]],
                    "orderby": "LINE asc,UPDATED_AT desc",
                    "limit": pageSize
                }
                return httpService.httpHelper(httpService.webApi.api, "tools/fbafeedetail", "index?page=" + currentPage, "POST", datas).then(function (datas) {
                    if (datas.data) {
                        $scope.fbaFeeDetailOptions.totalItems = datas._meta.totalCount;
                        $scope.fbaFeeDetailOptions.data = datas.data;
                    }
                    $scope.fbaFeeDetailOptions.paginationCurrentPage = currentPage;
                    $scope.fbaFeeDetailOptions.paginationPageSize = pageSize;
                });
            }
            ...
            $scope.fbaFeeDetailOptions = {
                ...
                onRegisterApi: function (gridApi) {
                    ...
                    gridApi.pagination.on.paginationChanged($scope, function (newPage, pageSize) {
                        if (getFbaFeeDetailPage) {
                            getFbaFeeDetailPage(newPage, pageSize);
                        }
                    });
                    //行选中事件
            ...
            //页码改变时触发方法
            function getFbaFeeDetailPage(currentPage, pageSize) {
                $scope.fbaFeeDetailInit(currentPage, pageSize);
            }
            ...
            $scope.fbaFeeDetailAdd = function (item) {
                ...
                httpService.httpHelper(httpService.webApi.api, "tools/fbafeedetail", "index", "POST", datas).then(function (datas) {
                    ...
                    line = line + 10;
                    var newData = {
                        "FBAFEERULE_ID": $scope.model.FBAFEERULE_ID,
                        "LINE": line,
                        "LENGTHMAX": 0,
                        "WIDTHMAX": 0,
                        "HEIGHTMAX": 0,
            ...
            //MCF费表
            $scope.mcfFeeDetailInit = function () {
                var pageSize = $scope.mcfFeeDetailOptions.paginationPageSize ? $scope.mcfFeeDetailOptions.paginationPageSize : 5;
                var currentPage = $scope.mcfFeeDetailOptions.paginationCurrentPage ? $scope.mcfFeeDetailOptions.paginationCurrentPage : 1;
                var datas = {
                    "where": ['and', ['=', 'FBAFEERULE_ID', $scope.model.FBAFEERULE_ID]],
                    "orderby": "LINE asc,UPDATED_AT desc",
                    "limit": pageSize
                }
                return httpService.httpHelper(httpService.webApi.api, "tools/mcffeedetail", "index?page=" + currentPage, "POST", datas).then(function (datas) {
                    if (datas.data) {
                        $scope.mcfFeeDetailOptions.totalItems = datas._meta.totalCount;
                        $scope.mcfFeeDetailOptions.data = datas.data;
                    }
                    $scope.mcfFeeDetailOptions.paginationCurrentPage = currentPage;
                    $scope.mcfFeeDetailOptions.paginationPageSize = pageSize;
                });
            }
            ...
            $scope.mcfFeeDetailOptions = {
                ...
                onRegisterApi: function (gridApi) {
                    ...
                    //分页按钮事件
                    gridApi.pagination.on.paginationChanged($scope, function (newPage, pageSize) {
                        if (getMcfFeeDetailInitPage) {
                            getMcfFeeDetailInitPage(newPage, pageSize);
                        }
                    });
                    //行选中事件
            ...
            //页码改变时触发方法
            function getMcfFeeDetailInitPage(currentPage, pageSize) {
                $scope.mcfFeeDetailInit(currentPage, pageSize);
            }
            ...
            //月存储&&LongTime费表
            $scope.mStorageFeeDetailInit = function () {
                var pageSize = $scope.mStorageFeeDetailOptions.paginationPageSize ? $scope.mStorageFeeDetailOptions.paginationPageSize : 5;
                var currentPage = $scope.mStorageFeeDetailOptions.paginationCurrentPage ? $scope.mStorageFeeDetailOptions.paginationCurrentPage : 1;
                var datas = {
                    "where": ['and', ['=', 'FBAFEERULE_ID', $scope.model.FBAFEERULE_ID]],
                    "orderby": "LINE asc,UPDATED_AT desc",
                    "limit": pageSize
                }
                return httpService.httpHelper(httpService.webApi.api, "tools/mstoragefeedetail", "index?page=" + currentPage, "POST", datas).then(function (datas) {
                    if (datas.data) {
                        $scope.mStorageFeeDetailOptions.totalItems = datas._meta.totalCount;
                        $scope.mStorageFeeDetailOptions.data = datas.data;
                    }
                    $scope.mStorageFeeDetailOptions.paginationCurrentPage = currentPage;
                    $scope.mStorageFeeDetailOptions.paginationPageSize = pageSize;
                });
            }
            ...
            //页码改变时触发方法
            function getMStorageFeeDetailInitPage(currentPage, pageSize) {
                $scope.mStorageFeeDetailInit(currentPage, pageSize);
            }
