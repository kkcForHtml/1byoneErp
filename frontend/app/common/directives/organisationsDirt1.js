define(['angularAMD','app/common/Services/commonService'],function(angularAMD) {
    "use strict";
    angularAMD.directive('organisationsDirt1', function ($q,commonService,httpService,configService) {
        return {
            restrict: 'A',
            scope: {
                options: "=",
            ...
            replace:true,
            template:"<select ng-model='selectModel' class='form-control input-sm' ng-options='item as item.ORGANISATION_NAME_CN for item in organizations' ng-change='change(selectModel)'></select>",
            link: function ($scope, element, attrs, ngModel) {
                ...
                $scope.change = function (selectModel) {
                    if($scope.options && $scope.options.change){
                        var os = $scope.organizations.filter(a=>a.ORGANISATION_CODE == selectModel);
