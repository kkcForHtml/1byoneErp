...
class Connection extends BaseClient implements ICoroutine
{
    //REDIS服务主机IP
    ...
    public function executeCommand($name, $params)
    {
        ...
        $client->setDefer();
        call_user_func_array([$client, $name], $params);
        $result = $client->recv();
        $this->release();
        return $result;
    }
    ...
    public function send($name, $db, $data)
    {
        ...
        if (in_array($redisCommand, $this->redisCommands)) {
            $client = $this->getRedis();
            $client->setDefer();
            call_user_func_array([$client, $name], $data);
            return $this;
        }
    }
