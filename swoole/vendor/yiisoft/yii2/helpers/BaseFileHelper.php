...
class BaseFileHelper
{
    ...
    public static function copyDirectory($src, $dst, $options = [])
    {
        ...
        while (($file = readdir($handle)) !== false) {
            ...
            if (static::filterPath($from, $options)) {
                if (isset($options['beforeCopy']) && !$GLOBALS['call_user_func']($options['beforeCopy'], $from, $to)) {
                    continue;
                }
                if (is_file($from)) {
                    if (!$dstExists) {
                ...
                if (isset($options['afterCopy'])) {
                    $GLOBALS['call_user_func']($options['afterCopy'], $from, $to);
                }
            }
        }
        closedir($handle);
    ...
    public static function filterPath($path, $options)
    {
        if (isset($options['filter'])) {
            $result = $GLOBALS['call_user_func']($options['filter'], $path);
            if (is_bool($result)) {
                return $result;
            }
        }
